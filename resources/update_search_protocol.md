...
- **Arm 2 — "Identification of studies via other methods":** the original exploratory Google Scholar searching, backward reference searching, forward citation ("Cited by") searching, and related-article suggestions. For this arm, report the stages that are genuinely reconstructable from the corpus record: sources assessed at full text (n = 34 retained in the corpus; additional assessed-but-rejected sources only if the author can enumerate them), assigned to the formal analytical sample (n = 21), retained as contextual/policy/decision-support sources (n = 8, including the user-approved editorial C06), retained as methodology sources (n = 5). For "records identified" in this arm, state **"not recorded (exploratory phase)"** in a footnote rather than inserting any number.
